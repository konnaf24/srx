...
| dnsmasq (`probe.lab`, authoritative only) | 53 | dns |
...
- `all` starts all 18 workloads concurrently. The sustained `wrk` and `iperf3`
  checks run for 60 seconds by default; use `all --duration <seconds>` to
...
./venv/bin/python deploy/srx_workload.py --target 10.10.10.45 dns  --qname probe.lab
./venv/bin/python deploy/srx_workload.py --target 10.10.10.45 eicar
./venv/bin/python deploy/srx_workload.py --target 10.10.10.45 iperf
